...
Модификатор использует `sharedBounds` с масштабированием без искажения пропорций и сменой содержимого между обрезанным превью и полной фотографией. Галерея создаёт layout изображения до завершения загрузки, использует memory-cache placeholder по URI и загружает оригинал для увеличения.

Системный predictive back обрабатывается навигационным host, а не галереей или `AppActivity`. Отмена возвращает общий прогресс в открытое состояние. Галерея сохраняет свои zoom и drag-to-dismiss. При увеличенном или перетаскиваемом изображении shared-перемещение отключено: overlay плавно исчезает без скачка к исходному масштабу.
...
При проверке этой части дополнительно запускайте `:feature:review-creation:impl:testDebugUnitTest`. На устройстве проверяйте обе точки входа, старт не с первой фотографии, перелистывание перед Back, отмену Back, zoom и drag-to-dismiss.
