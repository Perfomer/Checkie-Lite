...
Модификатор использует `sharedBounds` с масштабированием по ширине без искажения пропорций и сменой содержимого между обрезанным превью и полной фотографией. Масштабирование по высоте полного экрана чрезмерно увеличивало бы квадратное превью. Галерея создаёт стабильный полноэкранный layout изображения до завершения загрузки, вписывает в него фотографию через `ContentScale.Fit`, использует memory-cache placeholder по URI и загружает оригинал для увеличения. Размер layout не зависит от состояния загрузки painter.
...
Во время перехода clipping выполняется в координатах общего overlay, а не внутри масштабируемого содержимого. Поэтому радиус не увеличивается вместе с превью, скругление родительской карточки не теряется в первом кадре, а фиксированный clip исходного изображения не мешает получить финальную форму. Без совпадения модификатор применяет обычное локальное скругление.

Передача clipping между локальным изображением и overlay происходит в фазе рисования: локальный clip снимается только при наличии `clipPathInOverlay` для текущего кадра, а не по общему `isTransitionActive`. Анимируется доля перехода, и радиусы участников читаются при построении overlay-clip, после их регистрации. Это исключает старт анимации с временным fallback-радиусом ещё не зарегистрированного изображения.
