...
UI объявляет группу и ID содержимого, а регистрация пары экранов разрешает конкретные группы. Карточки относятся к `ReviewContent`, поле поиска — к `SearchFieldContent`. Вёрстка не знает о destination и не проверяет текущий экран.
...
navigation {
    ...
    sharedTransition<MainDestination, ReviewDetailsDestination>(groups = setOf(ReviewContent))
    sharedTransition<MainDestination, SearchDestination>(groups = setOf(SearchFieldContent))
}
```
...
- `MainDestination → SearchDestination` — переход от поискового поля к вводу запроса;
...
`SharedContentGroup` — контракт без Compose в `common:navigation:api:core`. Типизированные объекты `ReviewContent` и `SearchFieldContent` находятся в `presentation.transition` соответствующих feature API. Фреймворк не зависит от этих объектов. Вызов без `groups` разрешает только `DefaultSharedContentGroup`, а не все группы; это сохраняет поддержку существующего отдельного протокола изображений галереи.
...
Перед сменой стека host подготавливает пару на покоящемся исходном экране и пропускает два кадра: один для обновления композиции, второй для размещения с новым ключом. Только затем передаёт стек аниматору. Иначе смена ключа сбрасывает сохранённые Compose границы и элемент прыгает в конечное положение. Новая навигация ждёт завершения текущей анимации. Для predictive back такая же подготовка выполняется перед передачей начала жеста в Decompose, а события движения стоят в очереди.
...
```kotlin
SharedNavigationContent(group = ReviewContent, id = reviewId) {
    // Карточка или содержимое экрана деталей.
}
```

Этот блок ничего не рисует и не меняет layout. Он предоставляет потомкам контекст идентичности, группу и локальное условие `isEnabled`. Вложенный блок с новым ID наследует группу, если явно не задана другая. Общие виджеты не импортируют группы из feature.

Ключи элементов составные:

| Элемент | Ключ сопоставления |
| --- | --- |
| Текст и другие именованные элементы | Пара entries + группа + ID содержимого + значение `SharedNavigationElement` |
| Изображение | Пара entries + группа + ID содержимого + URI изображения |
| Контейнер | Пара entries + группа + ID содержимого + роль слоя: поверхность или содержимое |

Поэтому два отзыва с одинаковым заголовком или фотографией не связываются друг с другом, если у них разные ID. ID должен быть стабильным и одинаковым на обоих экранах. Разные группы и разные пары entries изолированы даже при одинаковых ID. Разрешённая группа не отменяет локальное `isEnabled`: например, обрезанная карточка по-прежнему исключается из сопоставления.
...
Тулбар и карусель галереи помечаются `Modifier.navigationForeground()`. Во время shared-перехода они рисуются в том же overlay, но выше фотографии: слой фото — `3F`, foreground — `4F`. Обычный `Modifier.zIndex` не может поднять содержимое экрана над отдельным shared-overlay. Модификатор сохраняет навигационную прозрачность внутри поднятого слоя, поскольку прозрачность предков на него больше не действует. Вне shared-перехода сохраняется обычная отрисовка экрана; дополнительная прозрачность не применяется. Модификатор ставится снаружи собственных анимаций и трансформаций элементов управления, чтобы они оставались частью поднятого слоя.
...
Overlay-slot также включает `handleBackButton`: Back, доставленный напрямую в Decompose, закрывает overlay, а не основной стек. Содержимое под overlay получает вложенный `NavigationEventDispatcherOwner`, отключённый на всё время отображения overlay, включая анимацию закрытия. Это блокирует его Compose Back-обработчики независимо от порядка регистрации: например, подтверждение выхода из редактора не перехватывает жест галереи. Сам overlay продолжает получать прогресс, завершение и отмену predictive Back. После удаления удерживаемого overlay нижняя область снова включается, сохраняя собственные обработчики экрана. Одного размещения обработчика overlay после `mainContent()` недостаточно: новый экран может зарегистрировать свой обработчик позже, когда навигационный хост уже существует.
