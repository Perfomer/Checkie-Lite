...
UI объявляет группу, роль и ID содержимого, а регистрация пары экранов задаёт сопоставление ролей. Карточки относятся к `ReviewContent`, поле поиска — к `SearchFieldContent`. Вёрстка не знает о destination и не проверяет текущий экран.
...
```kotlin
SharedNavigationContent(group = ReviewContent, role = ReviewListItem, id = reviewId) {
    // Карточка списка. На странице деталей у того же ID роль ReviewPage.
}
```

Этот блок ничего не рисует и не меняет layout. Он предоставляет потомкам контекст идентичности, группу, роль и локальное условие `isEnabled`. Вложенный блок с новым ID наследует группу и роль, если явно не заданы другие. При смене группы чужая роль не наследуется. Группу можно не указывать: она выводится из явной роли. Общие виджеты не импортируют группы и роли из feature.

Ключи элементов составные:

| Элемент | Ключ сопоставления |
| --- | --- |
| Текст и другие именованные элементы | Пара entries + канал сопоставления ролей + группа + ID + значение `SharedNavigationElement` |
| Изображение | Пара entries + канал сопоставления ролей + группа + ID + URI изображения |
| Контейнер | Пара entries + канал сопоставления ролей + группа + ID + слой: поверхность или содержимое |
