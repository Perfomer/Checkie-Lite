...
internal class ImageClipShapeTest {
    ...
    @Test
    fun `source corners follow centered content without scaling the radius`() {
        val shape = ImageClipShape(30.dp, IntSize(100, 200))
        val outline = shape.createOutline(Size(200F, 600F), LayoutDirection.Ltr, Density(1F)) as Outline.Rounded
        assertEquals(RoundRect(Rect(0F, 100F, 200F, 500F), CornerRadius(30F)), outline.roundRect)
    }

    @Test
    fun `taller target content clips to shared bounds using display density`() {
        val shape = ImageClipShape(30.dp, IntSize(100, 200))
    ...
    @Test
    fun `unmeasured content uses shared bounds and limits oversized corners`() {
        val shape = ImageClipShape(30.dp, IntSize.Zero)
